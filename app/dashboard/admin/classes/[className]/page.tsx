...
export default function ClassDetailPage() {
  ...
  const fetchSemesters = async () => {
    try {
      ...
      if (structure.length > 0) setSelectedSemesterId(structure[0].id);
    } catch {
  ...
  const handleExportClass = async () => {
    try {
      setIsExporting(true);
      const blob = await exportService.downloadStudentsXlsx(className);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `etudiants_${className.replace(/\s+/g, '_')}.xlsx`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
    } catch {
  ...
  const handleAddOrEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', email: '', password: '', birthDate: '', birthPlace: '', class: className, studentId: '' });
      fetchStudents();
    } catch (err: any) {
  ...
  const requestBulkDelete = () => {
    ...
    const targeted = classStudents.filter((s) => selectedIds.includes(s.id));
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showNotification('success', deleteTarget.ids.length > 1 ? `${deleteTarget.ids.length} étudiants supprimés` : 'Étudiant supprimé');
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-black text-slate-400 hover:text-primary uppercase tracking-widest mb-2 transition-all"
          >
          ...
          <p className="text-slate-400 text-sm font-medium">{classStudents.length} étudiant{classStudents.length !== 1 ? 's' : ''} dans cette classe.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <FileDown size={18} />
            <span className="hidden md:inline">Canevas</span>
          </button>
          <button
            onClick={handleExportClass}
            disabled={isExporting}
            className="glass-card p-2.5 md:px-4 md:py-2.5 text-sm font-bold text-slate-600 hover:text-emerald-600 transition-all flex items-center gap-2 border-white/60 disabled:opacity-50"
            title="Exporter cette classe"
          >
            {isExporting ? <Clock className="animate-spin" size={18} /> : <FileSpreadsheet size={18} />}
          ...
          `} title="Importer dans cette classe">
        ...
        <div className="p-6 border-b border-white/20 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/40">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-400 px-3 truncate">
              Affichage de {filteredStudents.length} sur {classStudents.length}
            </span>
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-20 text-center">
                    ...
                    <p className="text-slate-500 font-bold">Aucun étudiant dans cette classe</p>
                    <p className="text-slate-400 text-sm">Ajoutez un étudiant ou importez un fichier Excel.</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={student.id} className={`group border-b border-white/10 hover:bg-white/50 transition-all ${selectedIds.includes(student.id) ? 'bg-primary/5' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">Né(e) le {student.birthDate ? new Date(student.birthDate).toLocaleDateString() : 'N/A'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="glass-card w-full max-w-xl p-8 border-white/60 shadow-2xl animate-slide-up">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-black text-slate-800 tracking-tight">
                {editingStudent ? 'Modifier Étudiant' : `Nouvel Étudiant — ${className}`}
              </h3>
